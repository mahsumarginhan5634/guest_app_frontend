export default function PostContent(){
    return (
        <>
            <div className="bg-white rounded-lg shadow">
                <div className="p-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                            <div className="story-circle" style={{width: "40px", height: "40px"}}>
                                <div className="story-circle-inner">
                                    <svg
                                        className="w-full h-full text-blue-500"
                                        viewBox="0 0 20 20"
                                        fill="currentColor"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <circle cx="10" cy="10" r="10" fill="#EC4899"></circle>
                                        <text
                                            x="50%"
                                            y="50%"
                                            dominantBaseline="middle"
                                            textAnchor="middle"
                                            fill="white"
                                            fontSize="8"
                                            fontWeight="bold"
                                        >
                                            ZK
                                        </text>
                                    </svg>
                                </div>
                            </div>
                            <div>
                                <p className="font-semibold">Zeynep Kaya</p>
                                <p className="text-xs text-gray-500">2 saat önce</p>
                            </div>
                        </div>
                        <button className="text-gray-500 hover:text-gray-700">
                            <svg
                                className="w-6 h-6"
                                fill="currentColor"
                                viewBox="0 0 20 20"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <path
                                    d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z"></path>
                            </svg>
                        </button>
                    </div>
                    <div className="mt-3">
                        <p>
                            Bugün harika bir gün! İstanbul'da güneşli bir hafta sonu gezintisi 🌞
                            #HaftaSonu
                            #İstanbul
                        </p>
                    </div>
                </div>
                <div
                    className="bg-gradient-to-r from-blue-400 to-purple-500 aspect-w-16 aspect-h-9 relative"
                    style={{height: "300px"}}
                >
                    <svg
                        className="absolute inset-0 w-full h-full"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 800 600"
                    >
                        <rect width="800" height="600" fill="url(#grad1)"></rect>
                        <defs>
                            <linearGradient id="grad1" x1="0%" y1="0%" x2="100%" y2="0%">
                                <stop offset="0%"
                                      style={{stopColor: "#60A5FA", stopOpacity: 1}}></stop>
                                <stop offset="100%"
                                      style={{stopColor: "#A855F7", stopOpacity: 1}}></stop>
                            </linearGradient>
                        </defs>
                        <circle cx="600" cy="100" r="80" fill="#FCD34D" opacity="0.8"></circle>
                        <path
                            d="M0 400 Q 200 300 400 400 T 800 400 L 800 600 L 0 600 Z"
                            fill="#10B981"
                            opacity="0.7"
                        ></path>
                        <path
                            d="M0 450 Q 200 400 400 450 T 800 450 L 800 600 L 0 600 Z"
                            fill="#047857"
                            opacity="0.5"
                        ></path>
                    </svg>
                </div>
                <div className="p-4">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-2">
                            <button
                                className="flex items-center space-x-1 text-gray-500 hover:text-blue-600">
                                <svg
                                    className="w-5 h-5"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z"></path>
                                </svg>
                                <span>124</span>
                            </button>
                            <button
                                className="flex items-center space-x-1 text-gray-500 hover:text-red-600">
                                <svg
                                    className="w-5 h-5"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        fillRule="evenodd"
                                        d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z"
                                        clipRule="evenodd"
                                    ></path>
                                </svg>
                                <span>32</span>
                            </button>
                        </div>
                        <div className="text-gray-500 text-sm">
                            <span>42 yorum</span>
                        </div>
                    </div>
                    <div className="border-t border-b py-2">
                        <div className="flex justify-around">
                            <button
                                className="flex items-center space-x-2 post-interaction py-1 px-2">
                                <svg
                                    className="w-5 h-5"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z"></path>
                                </svg>
                                <span>Beğen</span>
                            </button>
                            <button
                                className="flex items-center space-x-2 post-interaction py-1 px-2">
                                <svg
                                    className="w-5 h-5"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        fillRule="evenodd"
                                        d="M18 5v8a2 2 0 01-2 2h-5l-5 4v-4H4a2 2 0 01-2-2V5a2 2 0 012-2h12a2 2 0 012 2zM7 8H5v2h2V8zm2 0h2v2H9V8z"
                                        clipRule="evenodd"
                                    ></path>
                                </svg>
                                <span>Yorum Yap</span>
                            </button>
                            <button
                                className="flex items-center space-x-2 post-interaction py-1 px-2">
                                <svg
                                    className="w-5 h-5"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        d="M15 8a3 3 0 00-2.995 2.824L12 11v1H7v2h5v1a3 3 0 002.824 2.995L15 18a3 3 0 002.995-2.824L18 15v-1h1v-2h-1v-1a3 3 0 00-2.824-2.995L15 8z"></path>
                                </svg>
                                <span>Paylaş</span>
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
}