...
export default function FollowDialog({ open, onClose, follows, followType }) {
    ...
    const handleProfileClick = (username) => {
        alert(`${username} profiline gidiliyor...`);
    };
    ...
    return (
        <>
           <Dialog
               ...
               maxWidth="xs"
               fullWidth
               className={"rounded-lg overflow-y-auto max-h-[500px]"}
           >
               <DialogTitle
                   className="bg-gradient-to-r from-blue-400 to-blue-500 text-white flex items-center justify-between py-4 px-6"
               >
                   <Typography variant="h6">
                       {
                           followType === FollowType.FOLLOWED ?  t("myFollowings") :
                               (followType === FollowType.FOLLOWER ? t("myFollowers") : followType)
                       }
                   </Typography>
                   <Box
                   ...
                   >
                       {follows.length + " "}
                       {
                           followType === FollowType.FOLLOWED ?  t("following") :
                               (followType === FollowType.FOLLOWER ? t("followers") : t("mutualFriend"))
                       }
                   </Box>
               </DialogTitle>
